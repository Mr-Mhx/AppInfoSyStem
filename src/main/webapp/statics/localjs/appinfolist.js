$("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"/categoryCon/queryCategoryLevel",//请求的url
...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href="/versionCon/appversionadd/"+obj.attr("appinfoid");
});
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="/appInfoCon/appinfomodify/"+ obj.attr("appinfoid");
	}else{
...
var saleSwitchAjax = function(appId,obj){
	$.ajax({
		type:"GET",
		url:"/appInfoCon/sale/"+$(this).attr("saleSwitch")+"/"+appId,
		dataType:"json",
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	window.location.href="/appInfoCon/appview/"+ obj.attr("appinfoid");
});
